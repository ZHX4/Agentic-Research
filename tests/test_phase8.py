...
from agentic_research.evaluation.metrics import (
...
from agentic_research.evaluation.validation import (
...
def retrieval_case(case_id: str = "c1", input_value: str | None = None) -> BenchmarkCase:
    return BenchmarkCase(
        case_id=case_id,
        kind="retrieval",
        input_hash=h(input_value or case_id),
        expected_ids=["a", "b"],
    )
...
def test_retrieval_benchmark_runs() -> None:
    result = evaluate_retrieval(
        [retrieval_case()],
        [PredictionRecord(case_id="c1", predicted_ids=["a", "b"])],
        system_name="test",
        benchmark_id="r",
    )
...
def test_prediction_integrity_rejects_unknown_and_duplicate_ids() -> None:
    ...
    with pytest.raises(ValueError):
        validate_prediction_coverage(
            cases, [PredictionRecord(case_id="c1"), PredictionRecord(case_id="c1")]
        )


def test_extraction_and_label_benchmarks() -> None:
    case = BenchmarkCase(
        case_id="c1", kind="extraction", input_hash=h("c1"), expected_fields={"method": "M"}
    )
    extraction = evaluate_extraction(
        [case],
        [PredictionRecord(case_id="c1", extracted_fields={"method": "M"})],
        system_name="test",
        benchmark_id="e",
    )
    assert extraction.metrics[0].value == 1.0
    assert (
        macro_field_f1([{"method": "M improves RAG"}], [{"method": "M improves retrieval"}]) == 0.0
    )
    gap_case = BenchmarkCase(
        case_id="g1", kind="gap", input_hash=h("g1"), expected_labels=["positive"]
    )
    labels = evaluate_labels(
        [gap_case],
        [PredictionRecord(case_id="g1", predicted_labels=["positive"])],
        kind="gap",
        system_name="test",
        benchmark_id="g",
    )
...
def test_temporal_benchmark_detects_leakage_and_unknown_rate() -> None:
    ...
    assert temporal_leakage(prediction.publication_years, 2020)["leakage_rate"] == pytest.approx(
        1 / 3
    )
...
def test_human_agreement_requires_two_annotators_and_unique_ratings() -> None:
    ratings = [
        HumanRating(case_id="1", annotator_id="a", label="good"),
        HumanRating(case_id="1", annotator_id="b", label="good"),
    ]
    ...
    with pytest.raises(ValueError):
        evaluate_human_ratings(
            [
                HumanRating(case_id="1", annotator_id="a", label="good"),
                HumanRating(case_id="1", annotator_id="a", label="bad"),
            ],
            evaluation_id="h3",
            task="gap_quality",
        )
...
def test_baseline_direction_and_ablation() -> None:
    ...
    ablation = evaluate_ablation(
        AblationSpec(ablation_id="a", component="retrieval", enabled=False, matched_case_ids=["1"]),
        {"score": 0.8},
        {"score": 0.7},
    )
...
def test_cost_summary() -> None:
    metrics = summarize_costs(
        [
            CostRecord(run_id="r1", wall_seconds=2, cpu_seconds=3, estimated_cost_usd=0.1),
            CostRecord(run_id="r2", wall_seconds=4, cpu_seconds=5, estimated_cost_usd=0.2),
        ]
    )
...
def test_bootstrap_is_deterministic() -> None:
    assert bootstrap_mean_ci([1.0, 2.0, 3.0], seed=42) == bootstrap_mean_ci(
        [1.0, 2.0, 3.0], seed=42
    )


def test_composite_report_is_content_addressed(tmp_path: Path) -> None:
