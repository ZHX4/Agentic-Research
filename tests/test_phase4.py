...
def _add_paper(
    ...
    *,
    ...
    world.upsert_node(
        WorldNode(node_id=paper_node, node_type="paper", paper_id=paper.paper_id, label=paper.title)
    )
    for field, node_type, edge_type, values in (
        ...
        for value in values:
            ...
            world.upsert_node(
                WorldNode(node_id=node_id, node_type=node_type, label=value)  # type: ignore[arg-type]
            )
            world.upsert_edge(
                WorldEdge(
...
def _add_claim(
    world: ScientificWorldModel, claim_id: str, paper_id: str, text: str, claim_type: str
) -> None:
    world.upsert_node(
        WorldNode(
            node_id=claim_id,
...
def test_missing_combination_is_candidate_only_and_uses_real_graph_ids(tmp_path: Path) -> None:
    with ScientificWorldModel(tmp_path / "world.sqlite") as world:
        _add_paper(
            world,
            _paper("p1", "A", 2024, {"domain": "vision"}),
            methods=["method-a"],
            datasets=["dataset-a"],
            tasks=["task-a"],
        )
        _add_paper(
            world,
            _paper("p2", "B", 2024, {"domain": "vision"}),
            methods=["method-b"],
            datasets=["dataset-b"],
            tasks=["task-a"],
        )
        result = discover_gaps(
            world, GapDiscoveryConfig(include_types={"missing_combination"}, min_entity_support=1)
        )
...
def test_method_task_missing_combination_is_found(tmp_path: Path) -> None:
    ...
    assert any(
        candidate.method == "method-a" and candidate.task == "task-b"
        for candidate in result.candidates
    )
...
def test_recurring_limitation_creates_candidate(tmp_path: Path) -> None:
    with ScientificWorldModel(tmp_path / "world.sqlite") as world:
        for paper_id in ("p1", "p2"):
            _add_paper(
                world, _paper(paper_id, paper_id, 2024, {}), methods=[], datasets=[], tasks=[]
            )
...
def test_underexplored_condition_is_detected(tmp_path: Path) -> None:
    with ScientificWorldModel(tmp_path / "world.sqlite") as world:
        ...
        result = discover_gaps(
            world,
            GapDiscoveryConfig(
                include_types={"underexplored_condition"},
...
def test_cross_domain_candidate_needs_missing_direct_combination(tmp_path: Path) -> None:
    with ScientificWorldModel(tmp_path / "world.sqlite") as world:
        _add_paper(
            world,
            _paper("p1", "Vision", 2024, {"domain": "computer vision"}),
            methods=["method-a"],
            datasets=[],
            tasks=[],
        )
        _add_paper(
            world,
            _paper("p2", "NLP", 2024, {"domain": "natural language processing"}),
            methods=[],
            datasets=[],
            tasks=["task-b"],
        )
        result = discover_gaps(
            world, GapDiscoveryConfig(include_types={"cross_domain"}, min_entity_support=1)
        )
    assert all(candidate.status == "candidate" for candidate in result.candidates)
...
def test_graph_negative_space_uses_common_task_neighbors(tmp_path: Path) -> None:
    with ScientificWorldModel(tmp_path / "world.sqlite") as world:
        ...
        result = discover_gaps(
            world,
            GapDiscoveryConfig(
                include_types={"graph_negative_space"}, min_graph_degree=2, min_common_neighbors=2
            ),
        )
    assert result.candidates
...
def test_temporal_cutoff_excludes_future_papers(tmp_path: Path) -> None:
    with ScientificWorldModel(tmp_path / "world.sqlite") as world:
        ...
        result = discover_gaps(
            world,
            GapDiscoveryConfig(
                include_types={"missing_combination"}, min_entity_support=1, temporal_cutoff=2022
            ),
        )
