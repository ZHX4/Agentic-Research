...
class ScientificWorldModel:
    ...
    def traverse(
        ...
        *,
        ...
        for _ in range(depth):
            ...
            params: list[object] = [*frontier, *frontier]
            query = (
                f"SELECT edge_id,source_id,target_id,edge_type FROM edges "
    ...
    def index_extraction(
        ...
        """Persist a Phase 2 extraction while preserving every provenance edge."""
        ...
        for section in extraction.sections:
            self.upsert_node(
                WorldNode(
                    ...
                    paper_id=paper.paper_id,
                    label=section.title,
                    payload={"level": section.level, "page_start": section.page_start, "page_end": section.page_end},
                )
            )
            self.upsert_edge(
                WorldEdge(
        ...
        for chunk in extraction.chunks:
            ...
            self.upsert_node(
                WorldNode(
                    ...
                    payload={"section_id": chunk.section_id, "page_start": chunk.page_start, "page_end": chunk.page_end},
                )
            )
            if chunk.section_id:
                self.upsert_edge(
        ...
        for claim, evidence in zip(extraction.claims, extraction.evidence, strict=True):
            self.upsert_node(
                WorldNode(
                    ...
                    paper_id=paper.paper_id,
                    label=claim.text,
                    payload={"chunk_id": claim.chunk_id, "raw_confidence": claim.raw_confidence, "calibrated_confidence": claim.calibrated_confidence},
                )
            )
            self.upsert_node(
                WorldNode(
                    ...
                    payload={"section": evidence.section, "page": evidence.page, "quote": evidence.quote, "source_locator": evidence.source_locator, "confidence": evidence.confidence},
                )
            )
            self.upsert_edge(
                WorldEdge(
                    edge_id=f"claim-evidence:{claim.claim_id}:{evidence.evidence_id}",
        ...
        for edge in extraction.citation_edges:
            ...
            self.upsert_edge(
                WorldEdge(
                    ...
                    target_id=target_id,
                    edge_type="cites",
                    payload={"confidence": edge.confidence, "marker": edge.marker, "context_chunk_id": edge.citation_context_chunk_id},
                )
            )
