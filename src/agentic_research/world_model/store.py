...
class ScientificWorldModel:
    ...
    def upsert_node(self, node: WorldNode) -> None:
        self.connection.execute(
            """INSERT INTO nodes(node_id,node_type,paper_id,label,payload_json)
               VALUES(?,?,?,?,?)
               ON CONFLICT(node_id) DO UPDATE SET node_type=excluded.node_type,
               paper_id=excluded.paper_id,label=excluded.label,
    ...
    def upsert_edge(self, edge: WorldEdge) -> None:
        self.connection.execute(
            """INSERT INTO edges(edge_id,source_id,target_id,edge_type,payload_json)
               VALUES(?,?,?,?,?)
               ON CONFLICT(edge_id) DO UPDATE SET source_id=excluded.source_id,
               target_id=excluded.target_id,edge_type=excluded.edge_type,
               payload_json=excluded.payload_json""",
            (
                edge.edge_id,
                edge.source_id,
    ...
    def upsert_chunk(
        ...
        *,
        ...
        self.connection.execute(
            """INSERT INTO chunks(
               chunk_id,paper_id,title,text,section,page_start,page_end,year,source,
               vector,vector_dim,vector_model
               )
               VALUES(?,?,?,?,?,?,?,?,?,?,?,?)
               ON CONFLICT(chunk_id) DO UPDATE SET paper_id=excluded.paper_id,
               title=excluded.title,text=excluded.text,section=excluded.section,
               page_start=excluded.page_start,page_end=excluded.page_end,year=excluded.year,
        ...
        *,
        ...
        for _ in range(depth):
            ...
            rows = self.connection.execute(
                "SELECT edge_id,source_id,target_id,edge_type FROM edges "
    ...
    def lexical_search(
        ...
        if filters:
            ...
            if filters.get("temporal_cutoff") is not None:
                conditions.append("c.year IS NOT NULL AND c.year <= ?")
                params.append(filters["temporal_cutoff"])
            if filters.get("paper_ids"):
                ids = list(cast("list[str]", filters["paper_ids"]))
                placeholders = ",".join("?" for _ in ids)
                conditions.append(f"c.paper_id IN ({placeholders})")
                params.extend(ids)
            if filters.get("sources"):
                sources = list(cast("list[str]", filters["sources"]))
        ...
        sql = (
            ...
            f"WHERE chunks_fts MATCH ? AND {' AND '.join(conditions)} "
    ...
    def dense_candidates(
        self, *, embedding_model: str, filters: dict[str, object] | None = None
    ) -> list[sqlite3.Row]:
        ...
        if filters:
            ...
            if filters.get("year_to") is not None:
                ...
                params.append(filters["year_to"])
            if filters.get("temporal_cutoff") is not None:
                conditions.append("year IS NOT NULL AND year <= ?")
                params.append(filters["temporal_cutoff"])
            if filters.get("paper_ids"):
                ids = list(cast("list[str]", filters["paper_ids"]))
                placeholders = ",".join("?" for _ in ids)
        ...
        return self.connection.execute(
            f"SELECT * FROM chunks WHERE {' AND '.join(conditions)}", params
        ).fetchall()
    ...
    def index_extraction(
        ...
        self.upsert_node(
            WorldNode(
                ...
                label=paper.title,
                payload={"year": paper.year, "doi": paper.doi, "arxiv_id": paper.arxiv_id},
            )
        )
        for section in extraction.sections:
            ...
            self.upsert_edge(
                WorldEdge(
                    edge_id=f"contains:{paper.paper_id}:{section.section_id}",
        ...
        for chunk in extraction.chunks:
            self.upsert_chunk(
                ...
                year=paper.year,
                source=str(paper.metadata.get("source")) if paper.metadata.get("source") else None,
                vector=(vectors or {}).get(chunk.chunk_id),
                vector_model=vector_model if chunk.chunk_id in (vectors or {}) else None,
            )
            self.upsert_node(
                WorldNode(
            ...
            if chunk.section_id:
                self.upsert_edge(
                    WorldEdge(
                        edge_id=f"contains:{chunk.section_id}:{chunk.chunk_id}",
        ...
        for ref in extraction.references:
            self.upsert_node(
                WorldNode(
                    node_id=ref.reference_id,
                    node_type="reference",
                    paper_id=paper.paper_id,
                    label=ref.title or ref.raw_text[:160],
            ...
            if edge.cited_paper_id:
                ...
                self.upsert_node(
                    WorldNode(
                        node_id=target_id,
                        node_type="paper",
                        paper_id=edge.cited_paper_id,
                        label=edge.cited_paper_id,
                        payload={"external": True, "identifier": edge.cited_paper_id},
                    )
                )
            else:
            ...
            self.upsert_edge(
                WorldEdge(
                    ...
                    payload={
                        ...
                        "marker": edge.marker,
                        "context_chunk_id": edge.citation_context_chunk_id,
                    },
                )
            )
        for author in paper.authors:
            ...
            self.upsert_edge(
                WorldEdge(
                    edge_id=f"authored-by:{paper.paper_id}:{node_id}",
                    source_id=paper_node_id,
                    target_id=node_id,
        ...
        for field, node_type, edge_type in (
            ("methods", "method", "has_method"),
            ("datasets", "dataset", "has_dataset"),
            ("metrics", "metric", "has_metric"),
            ("baselines", "baseline", "has_baseline"),
            ("tasks", "task", "has_task"),
        ):
            for value in getattr(paper, field):
                ...
                self.upsert_node(
                    WorldNode(
                        node_id=node_id,
                        node_type=cast(
                            "Literal['paper', 'section', 'chunk', 'claim', 'evidence', "
                            "'reference', 'method', 'dataset', 'metric', 'baseline', "
                            "'task', 'author']",
                            node_type,
                        ),
                        paper_id=None,
                ...
                self.upsert_edge(
                    WorldEdge(
                        ...
                        target_id=node_id,
                        edge_type=cast(
                            "Literal['contains', 'supports', 'qualifies', 'contradicts', "
                            "'contextualizes', 'cites', 'has_method', 'has_dataset', "
                            "'has_metric', 'has_baseline', 'has_task', 'authored_by', "
                            "'references']",
                            edge_type,
                        ),
                    )
