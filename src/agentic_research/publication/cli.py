...
from agentic_research.schemas.phase10 import ArtifactKind, ModelProviderDisclosure
...
def manifest(
    ...
    kinds: list[str] = typer.Option(
        [], help="Repeat per artifact: code|dataset|config|result|manuscript|environment|other."
    ),
    licenses: list[str] = typer.Option(
        [], help="Repeat per artifact SPDX id; empty string means unspecified."
    ),
    output: Path = typer.Option(...),
    environment_lock: str = typer.Option("pyproject.toml"),
) -> None:
    if len(source_commit) < 7:
        raise typer.BadParameter("--source-commit must be at least 7 characters")
    allowed_kinds = {"code", "dataset", "config", "result", "manuscript", "environment", "other"}
    ...
    for kind in kinds:
        if kind not in allowed_kinds:
            raise typer.BadParameter(f"--kinds must be one of {sorted(allowed_kinds)}")
    entries = []
    for index, path in enumerate(artifacts):
        license_value = (licenses[index] or None) if licenses else None
        entries.append(
            build_artifact_entry(
                path, cast(ArtifactKind, kinds[index]), f"artifact-{index:03d}", license_value
            )
        )
    package = build_reproducibility_package(
        ...
        entries,
        environment_lock,
        ["agentic-research-autonomous run", "agentic-research-evaluation report"],
        notes=["Verify all external datasets and third-party components before redistribution."],
    )
...
def audit_license(
    manifest_file: Path = typer.Option(..., exists=True, readable=True),
    output: Path = typer.Option(...),
) -> None:
...
def write_manuscripts(
    ...
    for manuscript in (system, benchmark, case):
        ...
        (output_dir / f"{manuscript.kind}.json").write_text(
            manuscript.model_dump_json(indent=2), encoding="utf-8"
        )
...
def bundle(
    ...
    package = ReproducibilityPackage.model_validate_json(
