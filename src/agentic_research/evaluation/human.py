    ...
    *,
    ...
    for rating in ratings:
        ...
        if pair in seen_pairs:
            raise ValueError(
                f"Duplicate rating for case {rating.case_id!r} by annotator {rating.annotator_id!r}"
            )
        seen_pairs.add(pair)
        ...
        for right_idx in range(left_idx + 1, len(annotators)):
            left = cast(
                ...
                [
                    row[left_idx]
                    for row in matrix
                    if row[left_idx] is not None and row[right_idx] is not None
                ],
            )
            right = cast(
                "list[str]",
    ...
    agreement = [
        MetricValue(
            name="mean_pairwise_cohen_kappa", value=mean(kappas) if kappas else 0.0, n=len(kappas)
        ),
        MetricValue(
            name="krippendorff_alpha_nominal",
            value=krippendorff_alpha_nominal(matrix),
            n=len(matrix),
        ),
    ]
    numeric = [rating.score for rating in ratings if rating.score is not None]
    aggregate = (
        [MetricValue(name="mean_human_score", value=mean(numeric), n=len(numeric))]
        if numeric
        else []
    )
