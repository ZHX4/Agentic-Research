...
from agentic_research.autonomy.reviewers import DeterministicReviewer, ReviewPanel
...
def adapters() -> list[StageAdapter]:
    def runner(name: str):
        def run(payload: dict[str, object]) -> dict[str, object]:
            ...
            return {"stage": name, "provenance_refs": sorted(set(refs)), "value": name}
        return run
...
def controller(tmp_path: Path, *, max_iterations: int = 1, reviewer: ReviewPanel | None = None) -> AutonomousController:
    ...
    panel = reviewer or ReviewPanel([DeterministicReviewer("r1"), DeterministicReviewer("r2")])
...
def test_run_completes_and_persists_checkpoints(tmp_path: Path) -> None:
...
def test_retry_is_bounded_and_recorded(tmp_path: Path) -> None:
    ...
    def flaky(_: dict[str, object]) -> dict[str, object]:
        ...
        return {"provenance_refs": ["ok"]}
    stage_map = {adapter.name: adapter for adapter in adapters()}
    stage_map["gap"] = StageAdapter("gap", flaky)
    store = SQLiteRunStore(tmp_path / "run.sqlite")
    ctl = AutonomousController(store, list(stage_map.values()), reviewers=ReviewPanel([DeterministicReviewer("r1")]))
    ctl.create("run-1", AutonomousRunConfig(max_iterations=1, max_stage_retries=1))
...
def test_reviewer_panel_is_independent_and_reports_provenance() -> None:
    panel = ReviewPanel([DeterministicReviewer("r1"), DeterministicReviewer("r2")])
