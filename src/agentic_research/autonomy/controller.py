...
def build_autonomous_report(state: AutonomousRunState) -> AutonomousRunReport:
    ...
    return AutonomousRunReport(
        report_id=f"autonomous-report:{digest}", run_id=state.run_id, final_status=state.status,
        iterations_completed=state.iteration, stage_count=len(state.stage_executions), review_count=len(state.reviews),
        critical_findings=critical, provenance_refs=sorted(set(state.provenance_refs)), output_artifacts=artifacts,
        warnings=warnings, stop_reason=state.stop_reason,
    )
...
class AutonomousController:
    ...
    def run(self, run_id: str, initial_payload: dict[str, Any]) -> AutonomousRunState:
        ...
        while state.iteration < state.config.max_iterations:
            ...
            for stage in self.ORDER:
                if stage == "review":
                    ...
                    if review_round.consensus == "reject" and state.config.stop_on_critical_review:
                        state.status = "failed"; state.stop_reason = "Reviewer rejection"; state.current_stage = None; self._persist(state); return state
                ...
                while execution.attempts < max_attempts:
                    ...
                    try:
                        ...
                        execution.input_sha256 = input_sha; execution.output_artifact = str(artifact_path); execution.output_sha256 = _sha256(artifact_path.read_text(encoding="utf-8")); execution.status = "succeeded"; payload = result; progressed = True; break
            ...
            if signature in state.progress_signatures and state.config.stop_on_no_progress:
                state.status = "completed"; state.stop_reason = "No progress detected"; state.current_stage = None; self._persist(state); return state
            state.progress_signatures.append(signature); state.iteration += 1
            if not progressed:
    ...
    def _persist(self, state: AutonomousRunState) -> None:
        state_json = state.model_dump_json(); self.store.save(state, _sha256(state_json))
    ...
    def _checkpoint(self, state: AutonomousRunState, stage: StageName) -> None:
        ...
        checkpoint = Checkpoint(
            ...
            run_id=state.run_id, iteration=state.iteration, last_completed_stage=stage,
            state_sha256="0" * 64, created_at=_now(),
        )
        state.checkpoints.append(checkpoint)
        checkpoint = checkpoint.model_copy(update={"state_sha256": _checkpoint_hash(state)})
        state.checkpoints[-1] = checkpoint
        self._persist(state)
...
def _identity(name: str) -> StageCallable:
    def runner(payload: dict[str, Any]) -> dict[str, Any]:
        provenance = list(payload.get("provenance_refs", [])); provenance.append(f"stage:{name}")
        return {"stage": name, "input": payload, "provenance_refs": sorted(set(provenance))}
    return runner
...
def run(run_id: str = typer.Option(...), state_db: Path = typer.Option(Path("artifacts/autonomy.sqlite")), input_file: Path = typer.Option(..., exists=True, readable=True), max_iterations: int = typer.Option(3, min=1, max=100), output: Path = typer.Option(...)) -> None:
    ...
    controller = AutonomousController(store, adapters, reviewers=panel)
    if store.load(run_id) is None: controller.create(run_id, AutonomousRunConfig(max_iterations=max_iterations))
    payload = json.loads(input_file.read_text(encoding="utf-8")); state = controller.run(run_id, payload); report = build_autonomous_report(state)
    output.parent.mkdir(parents=True, exist_ok=True); output.write_text(report.model_dump_json(indent=2), encoding="utf-8"); typer.echo(f"Wrote autonomous report {report.report_id} to {output}")
...
def resume(run_id: str = typer.Option(...), state_db: Path = typer.Option(Path("artifacts/autonomy.sqlite")), output: Path = typer.Option(...)) -> None:
    store = SQLiteRunStore(state_db); panel = ReviewPanel([DeterministicReviewer("reviewer-structural"), DeterministicReviewer("reviewer-provenance")])
    adapters = [StageAdapter(name, _identity(name)) for name in ("gap", "verify", "hypothesis", "execute", "evaluate", "report")]
    state = AutonomousController(store, adapters, reviewers=panel).resume(run_id); report = build_autonomous_report(state)
    output.parent.mkdir(parents=True, exist_ok=True); output.write_text(report.model_dump_json(indent=2), encoding="utf-8"); typer.echo(f"Wrote autonomous report {report.report_id} to {output}")
