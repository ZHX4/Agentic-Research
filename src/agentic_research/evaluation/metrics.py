...
def binary_classification_metrics(predicted: list[str], expected: list[str], positive: str) -> dict[str, float]:
    if len(predicted) != len(expected): raise ValueError("predicted and expected lengths must match")
    tp = sum(p == positive and e == positive for p, e in zip(predicted, expected)); fp = sum(p == positive and e != positive for p, e in zip(predicted, expected)); fn = sum(p != positive and e == positive for p, e in zip(predicted, expected)); tn = sum(p != positive and e != positive for p, e in zip(predicted, expected))
...
def cohen_kappa(labels_a: list[str], labels_b: list[str]) -> float:
    if len(labels_a) != len(labels_b): raise ValueError("rater label lengths must match")
    if not labels_a: return 0.0
    observed = mean(a == b for a, b in zip(labels_a, labels_b)); categories = sorted(set(labels_a) | set(labels_b)); p_a = Counter(labels_a); p_b = Counter(labels_b)
...
def krippendorff_alpha_nominal(ratings: list[list[str | None]]) -> float:
    observed_pairs = 0; disagreement = 0.0
    for row in ratings:
        values = [v for v in row if v is not None]
        for i in range(len(values)):
            for j in range(i + 1, len(values)):
                observed_pairs += 1; disagreement += 0.0 if values[i] == values[j] else 1.0
    if observed_pairs == 0: return 0.0
    Do = disagreement / observed_pairs; flat = [v for row in ratings for v in row if v is not None]; counts = Counter(flat); total = len(flat)
...
def bootstrap_mean_ci(values: list[float], *, samples: int = 2000, alpha: float = 0.05, seed: int = 0) -> tuple[float, float]:
    ...
    state = seed & 0xFFFFFFFF; draws: list[float] = []; n = len(values)
    for _ in range(samples):
        sample = []
        for _ in range(n):
            state = (1664525 * state + 1013904223) & 0xFFFFFFFF; sample.append(values[state % n])
        draws.append(mean(sample))
    draws.sort(); lo = draws[max(0, int((alpha / 2) * len(draws)) - 1)]; hi = draws[min(len(draws) - 1, int((1 - alpha / 2) * len(draws)))]
    return lo, hi


def temporal_leakage(prediction_years: dict[str, int | None], cutoff_year: int) -> dict[str, float]:
    if cutoff_year < 1900: raise ValueError("cutoff_year is invalid")
    total = len(prediction_years); future = sum(year is not None and year > cutoff_year for year in prediction_years.values()); unknown = sum(year is None for year in prediction_years.values())
