...
def similarity(a: str, b: str) -> float:
    x = set(re.findall(r"\w+", a.casefold(), flags=re.UNICODE))
    y = set(re.findall(r"\w+", b.casefold(), flags=re.UNICODE))
    return 1.0 if not x and not y else 0.0 if not x or not y else len(x & y) / len(x | y)


def cluster_hypotheses(
    items: list[HypothesisCandidate], threshold: float = 0.70
) -> list[list[HypothesisCandidate]]:
    if not 0.0 <= threshold <= 1.0:
        raise ValueError("threshold must be between 0 and 1")
    by_id = {x.hypothesis.hypothesis_id: x for x in items}
    remaining = set(by_id)
    ...
    while remaining:
        ...
        group: list[HypothesisCandidate] = []
        while queue:
            ...
            neighbors = sorted(
                i
                for i in remaining
                if similarity(current.hypothesis.statement, by_id[i].hypothesis.statement)
                >= threshold
            )
