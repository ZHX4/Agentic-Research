...
_LIMITATION_WORDS = re.compile(
    r"\b(limit(?:ation|ations)?|fail(?:s|ed|ure)?|cannot|unable|drawback|constraint)\b", re.I
)
_METHOD_WORDS = re.compile(
    r"\b(we propose|we introduce|we present|our method|our approach|we develop)\b", re.I
)
...
def extract_fields(chunks: list[TextChunk], sections: list[Section]) -> dict[str, list[str]]:
    ...
    for chunk in chunks:
        section_name = normalized_section.get(chunk.section_id or "", "")
    ...
    for chunk in chunks:
        for sentence in _sentences(chunk.text):
            ...
            claim_hash = hashlib.sha1(
                f"{paper_id}|{chunk.chunk_id}|{sentence}".encode()
            ).hexdigest()[:16]
            ...
            relation: ClaimRelation = "supports" if claim_type == "result" else "contextualizes"
            links.append(
                ClaimEvidenceLink(
...
def _candidate_values(field: str, sentence: str) -> list[str]:
    ...
    if field == "metrics":
        known = re.findall(
            r"\b(?:accuracy|precision|recall|f1(?:-score)?|auc|bleu|rouge(?:-\w+)?)\b",
            sentence,
            flags=re.I,
        )
    ...
    if field == "baselines":
        match = re.search(
            r"(?:compared (?:with|to)|baseline(?:s)?(?: include)?)\s+([^.;]+)", sentence, flags=re.I
        )
        return _clean_entity_list(match.group(1) if match else "")
    if field == "datasets":
        candidates = re.findall(
            r"(?:dataset|corpus|benchmark)\s*(?:called|named|:)?\s*([A-Z][A-Za-z0-9._-]*(?:\s+[A-Z][A-Za-z0-9._-]*){0,5})",
            sentence,
        )
        return _clean_entity_list(", ".join(candidates))
    match = re.search(
        r"(?:we propose|we introduce|we present|"
        r"our (?:method|approach)|we develop|using)\s+([^.;]+)",
        sentence,
        flags=re.I,
    )
    return _clean_entity_list(match.group(1) if match else "")
...
ClaimType = Literal["result", "method", "limitation", "dataset", "evaluation", "general"]
ClaimRelation = Literal["supports", "qualifies", "contradicts", "contextualizes"]


def _classify_claim(sentence: str, section_title: str) -> tuple[ClaimType | None, float]:
    ...
    if _METHOD_WORDS.search(sentence) or any(
        word in section for word in ("method", "approach", "architecture")
    ):
        return "method", 0.84 if _METHOD_WORDS.search(sentence) else 0.66
    if _RESULT_WORDS.search(sentence) or any(
        word in section for word in ("result", "experiment", "evaluation")
    ):
...
def _sentences(text: str) -> list[str]:
    ...
    if not normalized:
        return []
    return [
        item.strip() for item in re.split(r"(?<=[.!?])\s+(?=[A-Z0-9(])", normalized) if item.strip()
    ]
