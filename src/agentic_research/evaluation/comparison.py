...
def compare_baselines(
    ...
    *,
    ...
    common = (
        sorted(set.intersection(*(set(values) for values in all_systems.values())))
        if all_systems
        else []
    )
    if not common:
    ...
    winner = (max if direction == "higher" else min)(metrics, key=lambda name: metrics[name])
    deltas = {
        system: metrics[primary_name] - value
        for system, value in metrics.items()
        if system != primary_name
    }
...
def evaluate_ablation(
    spec: AblationSpec, baseline_metrics: dict[str, float], ablated_metrics: dict[str, float]
) -> AblationResult:
    ...
    return AblationResult(
        ...
        baseline_metrics={k: baseline_metrics[k] for k in common},
        ablated_metrics={k: ablated_metrics[k] for k in common},
        deltas=deltas,
...
def summarize_costs(costs: list[CostRecord]) -> list[MetricValue]:
    ...
    metrics = [
        MetricValue(
            ...
            value=sum(item.wall_seconds for item in costs),
            unit="seconds",
            n=len(costs),
        ),
        MetricValue(
            name="mean_wall_seconds",
            value=mean(item.wall_seconds for item in costs),
            unit="seconds",
            n=len(costs),
        ),
    ]
    for field, unit in (
        ...
        if vals:
            metrics.extend(
                [
                    MetricValue(name=f"total_{field}", value=sum(vals), unit=unit, n=len(vals)),
                    MetricValue(name=f"mean_{field}", value=mean(vals), unit=unit, n=len(vals)),
                ]
            )
    return metrics
...
def validate_baseline(spec: BaselineSpec) -> None:
    if spec.system_type == "oracle" and not spec.notes:
        raise ValueError(
            "Oracle baselines require an explicit note describing their information access"
        )
