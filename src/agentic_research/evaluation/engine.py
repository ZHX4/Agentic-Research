...
def evaluate_retrieval(cases: list[BenchmarkCase], predictions: list[PredictionRecord], *, system_name: str, benchmark_id: str, split: Literal["dev", "test"] = "test", k: int = 10) -> BenchmarkResult:
    by_id = {p.case_id: p for p in predictions}; ranked: list[list[str]] = []; gold: list[set[str]] = []; aps: list[float] = []; ndcgs: list[float] = []
    for case in cases:
        prediction = by_id.get(case.case_id, PredictionRecord(case_id=case.case_id)); ranked.append(prediction.predicted_ids); gold.append(set(case.expected_ids)); aps.append(average_precision_at_k(prediction.predicted_ids, set(case.expected_ids), k)); ndcgs.append(ndcg_at_k(prediction.predicted_ids, set(case.expected_ids), k))
    mrr = mean_reciprocal_rank(ranked, gold); precisions: list[float] = []; recalls: list[float] = []; f1s: list[float] = []
    for got, expected in zip(ranked, gold):
        p, r, f = precision_recall_f1(got[:k], expected); precisions.append(p); recalls.append(r); f1s.append(f)
...
def evaluate_extraction(cases: list[BenchmarkCase], predictions: list[PredictionRecord], *, system_name: str, benchmark_id: str, split: Literal["dev", "test"] = "test") -> BenchmarkResult:
    by_id = {p.case_id: p for p in predictions}; expected = [case.expected_fields for case in cases]; predicted = [by_id.get(case.case_id, PredictionRecord(case_id=case.case_id)).extracted_fields for case in cases]
...
def evaluate_labels(cases: list[BenchmarkCase], predictions: list[PredictionRecord], *, kind: Literal["gap", "novelty"], system_name: str, benchmark_id: str, positive: str | None = None, split: Literal["dev", "test"] = "test") -> BenchmarkResult:
    by_id = {p.case_id: p for p in predictions}; expected = [case.expected_labels[0] if case.expected_labels else "" for case in cases]
...
def evaluate_temporal(cases: list[BenchmarkCase], predictions: list[PredictionRecord], *, system_name: str, benchmark_id: str) -> BenchmarkResult:
    if not cases or any(case.cutoff_year is None for case in cases): raise ValueError("Every temporal case requires a cutoff_year")
    by_id = {p.case_id: p for p in predictions}; leakage: list[float] = []; unknown: list[float] = []; warnings: list[str] = []
    for case in cases:
        stats = temporal_leakage(by_id.get(case.case_id, PredictionRecord(case_id=case.case_id)).publication_years, case.cutoff_year or 0); leakage.append(stats["leakage_rate"]); unknown.append(stats["unknown_year_rate"])
        if stats["leakage_rate"] > 0: warnings.append(f"Temporal leakage detected in {case.case_id}")
...
def summarize_cost(costs: list[CostRecord]) -> list[MetricValue]:
    ...
    def avg(field: str) -> float | None:
        values = [getattr(item, field) for item in costs if getattr(item, field) is not None]; return mean(values) if values else None
    ...
    for field in ("cpu_seconds", "gpu_seconds", "peak_memory_mb", "input_tokens", "output_tokens", "estimated_cost_usd"):
        value = avg(field)
        if value is not None: metrics.append(MetricValue(name=f"mean_{field}", value=value, n=len(costs)))
    return metrics
