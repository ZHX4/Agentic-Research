...
from agentic_research.retrieval.embeddings import (
    EmbeddingProvider,
...
from agentic_research.schemas.phase4 import GapSignalType
...
def _build_embedder(
    kind: Literal["none", "hash", "sentence-transformers"], model_name: str
...
def _build_reranker(
    kind: Literal["none", "lexical", "cross-encoder"], model_name: str
) -> Reranker | None:
    if kind == "none":
        return None
    if kind == "lexical":
...
def acquire(
    ...
    output_dir: Path = typer.Option(
        Path("artifacts/fulltext"), help="Directory for downloaded files."
    ),
) -> None:
    ...
    try:
        ...
        for paper in load_papers(input):
            if (
                not paper.metadata.get("open_access_pdf_url")
                and not paper.arxiv_id
                and paper.url is None
            ):
...
def parse(
    manifest: Path = typer.Option(
        ..., exists=True, readable=True, help="JSONL full-text manifest."
    ),
...
def analyze(
    paper: Path = typer.Option(
        ..., exists=True, readable=True, help="JSON file containing one canonical Paper."
    ),
    ...
    calibration_model: Path | None = typer.Option(
        None, exists=True, readable=True, help="Optional isotonic calibration model JSON."
    ),
) -> None:
    ...
    output.write_text(
        json.dumps(
            {
                "paper": enriched.model_dump(mode="json"),
...
def index(
    input: Path = typer.Option(
        ...
        exists=True,
        readable=True,
        help="JSON produced by `analyze` containing paper and extraction.",
    ),
    database: Path = typer.Option(
        Path("artifacts/world-model.sqlite"), help="SQLite world-model database."
    ),
    embedding: Literal["none", "hash", "sentence-transformers"] = typer.Option(
        "hash", help="Embedding backend."
    ),
    embedding_model: str = typer.Option(
        "sentence-transformers/all-MiniLM-L6-v2", help="Sentence-transformers embedding model."
    ),
) -> None:
...
def retrieve(
    ...
    paper_id: list[str] = typer.Option(
        [], help="Restrict to these paper IDs; repeat for multiple IDs."
    ),
    source: list[str] = typer.Option(
        [], help="Restrict to these source labels; repeat for multiple labels."
    ),
...
def traverse(
    ...
    direction: Literal["out", "in", "both"] = typer.Option("both"),
    edge_type: list[str] = typer.Option(
        [], help="Restrict traversal to these edge types; repeat for multiple types."
    ),
) -> None:
    """Traverse the persistent scientific knowledge graph directionally."""
    with ScientificWorldModel(database) as world:
        result = world.traverse(
            node_id,
            depth=depth,
...
def discover_gaps_command(
    database: Path = typer.Option(
        Path("artifacts/world-model.sqlite"),
        exists=True,
        readable=True,
        help="Phase 3 scientific world-model database.",
    ),
    ...
    max_underexplored_coverage: float = typer.Option(0.2, min=0, max=1),
    max_candidates_per_type: int = typer.Option(200, min=1, max=10000),
    include_type: list[str] = typer.Option(
        [], help="Restrict detectors; repeat for multiple types."
    ),
) -> None:
    """Discover deterministic candidate gaps without novelty verification."""
    allowed_types = GapDiscoveryConfig().include_types
    for item in include_type:
        if item not in allowed_types:
            raise typer.BadParameter(f"--include-type must be one of {sorted(allowed_types)}")
    config = GapDiscoveryConfig(
        ...
        include_types=(
            {cast(GapSignalType, item) for item in include_type}
            if include_type
            else GapDiscoveryConfig().include_types
        ),
    )
    with ScientificWorldModel(database) as world:
    ...
    print(
        f"Wrote {len(result.candidates)} candidate gaps from "
        f"{result.corpus_paper_count} papers to {output}"
    )
    print(
        "[dim]Phase 4 produces candidates only; novelty and "
        "counterevidence verification belong to Phase 5.[/dim]"
    )
...
def calibrate(
    input: Path = typer.Option(
        ..., exists=True, readable=True, help="JSONL with raw_confidence and correct fields."
    ),
    ...
    examples = [
        CalibrationExample.model_validate(json.loads(line))
