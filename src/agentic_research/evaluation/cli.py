...
def _read_json_list(path: Path, model):
...
def human(
    ...
    task: Literal["gap_quality", "novelty_verdict", "extraction_quality", "hypothesis_quality"] = typer.Option(...),
) -> None:
...
def baseline(
    ...
    result = compare_baselines(primary_name, {str(k): float(v) for k, v in primary_payload.items()}, {str(name): {str(k): float(v) for k, v in values.items()} for name, values in baseline_payload.items()}, comparison_id=comparison_id)
    _write(output, result.model_dump(mode="json"))
...
def summarize_cost(
    costs: Path = typer.Option(..., exists=True, readable=True),
    output: Path = typer.Option(...),
) -> None:
    records = _read_json_list(costs, CostRecord)
    _write(output, [metric.model_dump(mode="json") for metric in summarize_costs(records)])
