...
def reflect(h: Hypothesis) -> HypothesisReflection:
    ...
    return HypothesisReflection(
        ...
        hidden_assumptions=h.assumptions or ["The proposed mechanism can be isolated from implementation artifacts."],
...
def generate_candidates(gaps: list[GapCandidate], config: HypothesisConfig | None = None) -> list[HypothesisCandidate]:
    ...
    for idx, (left, right) in enumerate(itertools.combinations(source, 2)):
        if idx >= cfg.max_composed_pairs or set(left.source_gap_ids) & set(right.source_gap_ids):
            break
        ...
        item = _build(synthetic, "gap_composed", f"Combine: {left.mechanism} AND {right.mechanism}", "an incremental interaction effect", "reject if the joint system does not outperform both constituents", sorted(set(left.assumptions + right.assumptions)), ["Interaction survives matched-control analysis."], idx)
