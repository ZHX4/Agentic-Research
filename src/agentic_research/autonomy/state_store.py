...
class SQLiteRunStore:
    ...
    def __init__(self, path: Path) -> None:
        ...
        with self._connect() as db:
            ...
            db.execute(
                ...
                "run_id TEXT NOT NULL, checkpoint_json TEXT NOT NULL, "
    ...
    def save_checkpoint(
        self, checkpoint: Checkpoint, state: AutonomousRunState, state_sha256: str
    ) -> None:
        ...
        with self._connect() as db:
            db.execute(
                "INSERT INTO runs VALUES (?, ?, ?) ON CONFLICT(run_id) DO UPDATE SET "
                "state_json=excluded.state_json, state_sha256=excluded.state_sha256",
                (state.run_id, snapshot, state_sha256),
            )
            db.execute(
    ...
    def load(self, run_id: str) -> tuple[AutonomousRunState, str] | None:
        with self._connect() as db:
            ...
            cp = db.execute(
                "SELECT checkpoint_json, state_snapshot_json FROM checkpoints "
        ...
        if cp:
            ...
            if (
                checkpoint.checkpoint_id != snapshot.checkpoints[-1].checkpoint_id
                or self.checkpoint_hash(snapshot) != checkpoint.state_sha256
            ):
    ...
    def list_run_ids(self) -> list[str]:
        with self._connect() as db:
            return [
                str(row[0])
                for row in db.execute("SELECT run_id FROM runs ORDER BY rowid").fetchall()
            ]
