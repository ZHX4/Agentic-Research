        ...
        for page_index, page in enumerate(document):
            ...
            for block_index, raw in enumerate(payload.get("blocks", [])):
                ...
                if not text:
                    continue
                digest = hashlib.sha1(
                    f"{page_index + 1}|{block_index}|{raw.get('bbox')}|{text}".encode()
                ).hexdigest()[:16]
                blocks.append(
                    TextBlock(
                        block_id=f"b-{digest}",
        ...
        for page_index, page in enumerate(document):
            ...
            for index, table in enumerate(finder.tables):
                ...
                if not rows:
                    continue
                digest = hashlib.sha1(
                    f"{paper_id}|{page_index + 1}|{index}|{rows}".encode()
                ).hexdigest()[:16]
                tables.append(
                    TableRecord(
        ...
        for page_index, page in enumerate(document):
            ...
            for index, image in enumerate(image_infos):
                ...
                identity = hashlib.sha1(
                    f"{paper_id}|{page_index + 1}|{index}|{digest}|{bbox_values}".encode()
                ).hexdigest()[:16]
                figures.append(
                    FigureRecord(
...
def _nearest_caption(
    blocks: list[TextBlock], image_bbox: tuple[float, float, float, float]
) -> str | None:
    ...
    for block in blocks:
        if not re.match(r"^(figure|fig\.)\s*\d+\b", block.text, flags=re.IGNORECASE):
            continue
        distance = (
            min(abs(block.bbox.y0 - y1), abs(y0 - block.bbox.y1)) + abs(block.bbox.x0 - x0) * 0.05
        )
        if distance < 120:
