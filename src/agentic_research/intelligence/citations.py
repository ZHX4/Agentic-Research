...
_NUMERIC_RANGE = re.compile(r"\[(\d{1,4})(?:\s*[-–]\s*(\d{1,4}))(?:\s*,\s*\d+)*\]")
...
def extract_citation_edges(
    ...
    for chunk in chunks:
        markers = _citation_numbers(chunk.text)
        for number, marker in markers:
            ...
            edges[edge_id] = CitationEdge(
                ...
                marker=marker,
                confidence=0.93 if ref.order is not None else 0.70,
            )
...
def _guess_title(raw: str) -> str | None:
    ...
    if len(parts) < 2:
        return None
    title = max(parts[1:-1] or parts[1:], key=len)
    return title[:300]
